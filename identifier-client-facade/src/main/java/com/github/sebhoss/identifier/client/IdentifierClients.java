...
/**
 * Constructs new facades for future usage. All returned facades are
 * thread-safe.
 */
public final class IdentifierClients {
  ...
  public static IdentifierClient http() {
    return http("https://id.metio.wtf/"); //$NON-NLS-1$
  }

  /**
   * @param baseUrl
   *          The remote base-URL to talk to. Since this is used as the basis to
   *          construct URLs, the given base-URL is required to end in a "/"
   ...
   *          The remote base-URL to talk to. Since this is used as the basis to
   *          construct URLs, the given base-URL is required to end in a "/"
   * @return A new client facade which talks to the given baseUrl using the given
   *         HTTP client
   */
